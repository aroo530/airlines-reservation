...
class TripOperations {
  ...
  async getSubFlights(source, destination) {
    try {
      const connection = await pool.connect();
      const result = await connection.query(
        "SELECT * FROM Trips INNER JOIN Airline on trips.Airline_id = Airline.Airline_id WHERE Source Like $1 And Destination like $2 Order by Departure_date,Daparture_time,Arrival_date,Arrival_time",
        [source, destination]
      );
      connection.release();
      return result;
